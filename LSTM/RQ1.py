...
RQ_path="/data/zhujz/SyntaxLineDP/LSTM/RQ1/"
word2vec_path="/data/zhujz/SyntaxLineDP/LSTM/word2vec/"
...
def train():
    ...
    for project in projects:
        ...
        device = torch.device("cuda:0" if cuda_condiction else "cpu")
        print(device)
                ...
                with open(RQ_path+"lstm_result.txt","a") as f:
                    try:
                        f.write(str(epoch+1)+" recall@20%LOC:"+str(line_result["recall@20%LOC"])
                                +" effort@20%Recall:"+str(line_result["effort@20%Recall"])+"\n")
                    except:
                        f.write(str(epoch+1)+"\n")
