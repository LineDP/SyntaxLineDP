...
class BPE(object):
    def __init__(self, corpus_path):
        print("Read in files")
        print('==========')
        vocab = self.get_vocab(corpus_path)
        ...
        sorted_tokens_tuple = sorted(self.tokens_frequencies.items(),
                                     key=lambda item: (self.measure_token_length(item[0]), item[1]), reverse=True)
    ...
    def get_vocab(self, corpus_path):
        vocab = collections.Counter()
        files = os.listdir(corpus_path)

        for index, file in enumerate(files):
            df=pd.read_csv(corpus_path+file)
            lines=list(df['ast'])
            for line in tqdm(lines):
                if line =='[]':
                    continue
                ast_node=line[1:-1].split(',')
    ...
    def tokenize_word(self, string, sorted_tokens, unknown_token='<unk>'):
        ...
        for i in range(len(sorted_tokens)):
            ...
            for substring_end_position in substring_end_positions:
                ...
                string_tokens += self.tokenize_word(string=substring, sorted_tokens=sorted_tokens[i + 1:],
...
class Vocab(TorchVocab):
    def __init__(self, counter, max_size=None, min_freq=1):
        self.pad_index = 0
        self.unk_index = 1
        self.sos_index_java = 2
        self.eos_index = 3
        self.mask_index = 4
        self.cls_index = 5
        # <mask> is automatically included when creating a vocab
        super().__init__(counter,
                         specials=["<pad></w>", "<unk></w>", "<sosj></w>", "<eos></w>", "<cls></w>"],
...
def build():
    ...
    parser.add_argument("-c", "--corpus_path", type=str)
    parser.add_argument("-o", "--output_path", type=str)
    parser.add_argument("-s", "--vocab_size", type=int, default=50000)
    ...
    # Create BPE object
    corpus_path="/data/zhujz/linedp/Datasets/preprocessed_data7/"
    output_path="/data/zhujz/linedp/LSTM/vocaburary/"
    BPE_object = BPE(corpus_path=corpus_path)
    
    vocab = TokenVocab(BPE_object, max_size=args.vocab_size, min_freq=args.min_freq)

    print("VOCAB SIZE:", len(vocab))
    vocab.save_vocab(output_path+"vocab")
